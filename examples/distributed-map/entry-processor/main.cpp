...
main()
{
    ...
    std::cout << "The result after employees.execute_on_entries call is:"
              << std::endl;
    ...
    std::cout << "The result after employees.executeOnKeys call is:"
              << std::endl;
    for (auto& entry : result) {
        std::cout << entry.first << " salary: " << *entry.second << std::endl;
    }
    ...
    if (future.wait_for(std::chrono::seconds(1)) ==
        ...
        std::cout
          << "Could not get the result of submitToKey in 1 second for Mark"
          << std::endl;
    }
