 ...
 */
boost::future<client::hazelcast_client> HAZELCAST_API
new_client();
...
namespace client {
/*
 * You can use native C++ Client to connect to hazelcast nodes and make almost
 * all operations that a node does. Different from nodes, clients do not hold
 * data.
 *
 * Some of features of C++ Clients are:
 * * Access to distributed data structures like IMap, IQueue, MultiMap, ITopic
 * etc... For complete list see the classes extending DistributedObject
 * * Access to transactional distributed data structures like TransactionalMap,
 * TransactionalQueue etc...
 * * Ability to add cluster listeners to a cluster and entry/item listeners to
 * distributed data structures.
 * @see MembershipListener, IMap#add_entry_listener , IQueue#add_item_listener
 * etc .
 * * C++ Client is smart by default, which means that it knows where the data is
 * and asks directly to correct node. Note that you can turn this feature off (
 * client_config#setSmart), if you don't want your clients to connect every
 * node.
 *
...
public:
     ...
     */
    boost::shared_future<std::shared_ptr<multi_map>> get_multi_map(
      const std::string& name)
    {
        return client_impl_->get_distributed_object<multi_map>(name);
    }

    boost::shared_future<std::shared_ptr<replicated_map>> get_replicated_map(
      const std::string& name)
    {
     ...
     */
    boost::shared_future<std::shared_ptr<iqueue>> get_queue(
      const std::string& name)
    {
     ...
     */
    boost::shared_future<std::shared_ptr<ilist>> get_list(
      const std::string& name)
    {
     ...
     */
    boost::shared_future<std::shared_ptr<itopic>> get_topic(
      const std::string& name)
    {
     ...
     * @return distributed topic instance with the specified name
     */
    boost::shared_future<std::shared_ptr<reliable_topic>> get_reliable_topic(
      const std::string& name)
    {
    ...
    /**
     * Returns a generator that creates a cluster-wide unique IDs. Generated IDs
     * are {@code long} primitive values and are k-ordered (roughly ordered).
     * IDs are in the range from {@code 0} to {@code Long.MAX_VALUE}. <p> The
     * IDs contain timestamp component and a node ID component, which is
     ...
     * the generator safe even in split-brain scenario (for caveats,
     * {@link
     * com.hazelcast.internal.cluster.ClusterService#getMemberListJoinVersion()
     * see here}). <p> For more details and caveats, see class documentation for
     * {@link flake_id_generator}. <p>
     *
     * @param name name of the {@link flake_id_generator}
     * @return flake_id_generator for the given name
     */
    boost::shared_future<std::shared_ptr<flake_id_generator>>
    get_flake_id_generator(const std::string& name)
    {
     ...
     */
    boost::shared_future<std::shared_ptr<pn_counter>> get_pn_counter(
      const std::string& name)
    {
     ...
     */
    boost::shared_future<std::shared_ptr<ringbuffer>> get_ringbuffer(
      const std::string& name)
    {
     ...
     * Executor service enables you to run your <tt>Runnable</tt>s and
     * <tt>Callable</tt>s on the Hazelcast cluster. <p> <p><b>Note:</b> Note
     * that it doesn't support {@code invokeAll/Any} and doesn't have standard
     * shutdown behavior</p>
     *
     * @param name name of the executor service
     * @return the distributed executor service for the given name
     */
    boost::shared_future<std::shared_ptr<iexecutor_service>>
    get_executor_service(const std::string& name)
    {
    ...
    /**
     * Creates a new transaction_context associated with the current thread
    ...
    /**
     * Creates a new transaction_context associated with the current thread with
     * given options.
     *
     ...
     * <p>
     * LifecycleService allows you to shutdown this HazelcastInstance and listen
    ...
    /**
     *
     * @return the CP subsystem that offers a set of in-memory linearizable data
     * structures
     */
