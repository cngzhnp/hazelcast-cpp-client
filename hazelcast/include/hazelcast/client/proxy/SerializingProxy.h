...
protected:
    ...
    template<typename T>
    inline boost::future<std::optional<T>> to_object(
      boost::future<std::optional<serialization::pimpl::data>> f)
    {
        return f.then(
          boost::launch::sync,
          [this](boost::future<std::optional<serialization::pimpl::data>> f) {
              return to_object<T>(*(f.get()));
          });
    }

    template<typename T>
    boost::future<std::optional<T>> to_object(
    ...
    template<typename K, typename V>
    inline boost::future<std::optional<entry_view<K, V>>>
    to_object_entry_view(
      boost::future<std::optional<map::data_entry_view>> data_future)
    {
        return data_future.then(
          boost::launch::sync,
          [this](boost::future<std::optional<map::data_entry_view>> f) {
              auto dataView = f.get();
              if (!dataView) {
                  return std::optional<entry_view<K, V>>();
              }
...
protected:
    template<typename T>
    static boost::future<std::optional<T>> decode_optional_var_sized(
...
private:
    ...
    template<typename T>
    inline std::optional<T> to_object(const serialization::pimpl::data& data)
    {
    ...
    template<typename T>
    inline std::optional<T> to_object(
    ...
    template<typename T>
    inline std::optional<T> to_object(
      const std::optional<serialization::pimpl::data>& data)
    {
        return to_object<T>(data.value());
    }
};

template<>
boost::future<std::optional<serialization::pimpl::data>> HAZELCAST_API
SerializingProxy::invoke_and_get_future(protocol::ClientMessage& request);

template<>
boost::future<std::optional<map::data_entry_view>> HAZELCAST_API
SerializingProxy::invoke_and_get_future(protocol::ClientMessage& request,
                                        const serialization::pimpl::data& key);
