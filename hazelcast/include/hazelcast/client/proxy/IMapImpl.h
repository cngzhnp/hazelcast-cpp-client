...
public:
     ...
     * all the local dirty entries by calling MapStore.storeAll() and/or
     * MapStore.deleteAll()
     */
     ...
     */
    boost::future<protocol::ClientMessage> remove_interceptor(
      const std::string& id);

    /**
     * Evicts all keys from this map except locked ones.
     * <p/>
     * If a <tt>MapStore</tt> is defined for this map, deleteAll is
     * <strong>not</strong> called by this method. If you do want to deleteAll
     * to be called use the #clear() method. <p/> The EVICT_ALL event is fired
     * for any registered listeners. See EntryListener#map_evicted(MapEvent)}.
     *
     * @see #clear()
     */
...
protected:
    ...
    boost::future<protocol::ClientMessage> delete_entry(
      const serialization::pimpl::data& key);
    ...
    boost::future<protocol::ClientMessage> put_transient(
      const serialization::pimpl::data& key,
      const serialization::pimpl::data& value,
      std::chrono::milliseconds ttl);
    ...
    boost::future<std::optional<serialization::pimpl::data>> replace_data(
      const serialization::pimpl::data& key,
      const serialization::pimpl::data& value);

    boost::future<protocol::ClientMessage> set(
      const serialization::pimpl::data& key,
      const serialization::pimpl::data& value,
      std::chrono::milliseconds ttl);

    boost::future<protocol::ClientMessage> lock(
      const serialization::pimpl::data& key);
    ...
    boost::future<protocol::ClientMessage> unlock(
      const serialization::pimpl::data& key);

    boost::future<protocol::ClientMessage> force_unlock(
      const serialization::pimpl::data& key);
    ...
    boost::future<std::optional<map::data_entry_view>> get_entry_view_data(
      const serialization::pimpl::data& key);

    boost::future<bool> evict(const serialization::pimpl::data& key);

    boost::future<EntryVector> get_all_data(
      int partition_id,
    ...
    boost::future<protocol::ClientMessage> add_index_data(
      const config::index_config& config);

    boost::future<protocol::ClientMessage> put_all_data(
      int partition_id,
    ...
    std::pair<size_t, size_t> update_anchor(
      std::vector<std::pair<K, std::optional<V>>>& entries,
      ...
      int nearest_page)
    {
        ...
        for (size_t i = pageSize; i <= size && nearest_page < page;
             i += pageSize) {
            V* value = entries[i - 1].second
                         ? new V(entries[i - 1].second.value())
                         : nullptr;
...
private:
    std::shared_ptr<impl::ClientLockReferenceIdGenerator>
      lock_reference_id_generator_;

    class MapEntryListenerWithPredicateMessageCodec
    ...
    {
    public:
        MapEntryListenerMessageCodec(std::string name,
                                     bool include_value,
                                     int32_t listener_flags);
    ...
    std::shared_ptr<spi::impl::ListenerMessageCodec>
    create_map_entry_listener_codec(bool include_value,
                                    serialization::pimpl::data&& predicate,
                                    int32_t listener_flags);
      ...
      boost::future<protocol::ClientMessage> f) const
    {
        ...
        return std::make_pair(std::move(entries_data.value()),
                              std::move(anchor_list));
    }
};
} // namespace proxy
} // namespace client
