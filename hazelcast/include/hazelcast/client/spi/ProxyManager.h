...
public:
    typedef std::unordered_map<
      ...
      boost::shared_future<std::shared_ptr<ClientProxy>>>
      proxy_map;

    explicit ProxyManager(ClientContext& context);

    template<typename T>
    boost::shared_future<std::shared_ptr<T>> get_or_create_proxy(
      ...
      const std::string& id)
    {
        ...
        auto client_proxy =
          std::static_pointer_cast<ClientProxy>(concrete_proxy);
        auto proxy_future =
          initialize(client_proxy)
...
private:
    boost::future<void> initialize(
      const std::shared_ptr<ClientProxy>& client_proxy);

    template<typename T>
    boost::shared_future<std::shared_ptr<T>> convert_to_concrete_proxy_future(
      boost::shared_future<std::shared_ptr<ClientProxy>> proxy_future)
    {
        return proxy_future.then(
          boost::launch::sync,
