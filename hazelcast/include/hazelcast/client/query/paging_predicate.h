...
/**
 * NOTE: paging_predicate can only be used with values(), keySet() and entries()
 * methods!!!
 *
 * This class is a special Predicate which helps to get a page-by-page result of
 * a query. It can be constructed with a page-size, an inner predicate for
 * filtering, and a comparator for sorting. This class is not thread-safe and
 * stateless. To be able to reuse for another query, one should call
...
private:
    ...
    std::optional<serialization::pimpl::data> comparator_data_;
    std::optional<serialization::pimpl::data> predicate_data_;

    /**
     * Construct with a pageSize
     * results will not be filtered
     * results will be natural ordered
     * throws illegal_argument {@link illegal_argument} if pageSize is not
     * greater than 0
     *
     ...
     * results will be natural ordered
     * throws illegal_argument {@link illegal_argument} if pageSize is not
     * greater than 0 throws illegal_argument {@link illegal_argument} if inner
     * predicate is also {@link paging_predicate}
     *
     * @param predicate the inner predicate through which results will be
     * filtered
      ...
      , iteration_type_(iteration_type::VALUE)
    {
        ...
        out_stream_.write_object<bool>(nullptr);
        predicate_data_ =
          serialization_service.to_data<INNER_PREDICATE>(predicate);
    }
      ...
      , iteration_type_(iteration_type::VALUE)
    {
        ...
        comparator_ =
          std::make_shared<COMPARATOR>(std::forward<COMPARATOR>(comp));
    }
     ...
     * throws {@link illegal_argument} if inner predicate is also {@link
     * paging_predicate}
     *
     * @param predicate  the inner predicate through which results will be
     * filtered
...
template<typename K, typename V>
struct hz_serializer<query::paging_predicate<K, V>>
  : public identified_data_serializer
{
    ...
    {
        return static_cast<int32_t>(
          query::predicate_data_serializer_hook::PAGING_PREDICATE);
    }
     ...
     */
    static void write_data(const query::paging_predicate<K, V>& obj,
                           object_data_output& out)
    {
        ...
        out.write<std::string>(
          obj.IterationNames[static_cast<int32_t>(obj.iterationType)]);
