 ...
 */
template<typename K = serialization::pimpl::data,
         typename V = serialization::pimpl::data>
class NearCachedClientMapProxy
  : public imap
  , public std::enable_shared_from_this<NearCachedClientMapProxy<K, V>>
{
public:
    NearCachedClientMapProxy(const std::string& instance_name,
...
protected:
    ...
    void on_initialize() override
    {
        ...
        int partitionCount =
          this->get_context().get_partition_service().get_partition_count();
        near_cache_ =
          nearCacheManager
            .get_or_create_near_cache<K, V, serialization::pimpl::data>(
              spi::ClientProxy::get_name(), near_cache_config_);

        near_cache_ = impl::nearcache::InvalidationAwareWrapper<
          serialization::pimpl::data,
          V>::as_invalidation_aware(near_cache_, partitionCount);
        ...
        local_map_stats_ =
          monitor::impl::LocalMapStatsImpl(near_cache_->get_near_cache_stats());
    }
    ...
    void post_destroy() override
    {
        try {
            ...
            spi::ClientProxy::get_context()
              .get_near_cache_manager()
              .destroy_near_cache(spi::ClientProxy::get_name());
    ...
    boost::future<bool> contains_key_internal(
    ...
    boost::future<std::optional<serialization::pimpl::data>> get_internal(
      const serialization::pimpl::data& key_data) override
    {
        auto key = std::make_shared<serialization::pimpl::data>(key_data);
        ...
        if (cached) {
            ...
            return boost::make_ready_future(std::make_optional(*cached));
        }
            ...
            if (marked) {
                return future.then(
                  ...
                  [=](boost::future<std::optional<serialization::pimpl::data>>
                      ...
                      auto cachedValue =
                        data
                          ? std::make_shared<serialization::pimpl::data>(*data)
    ...
    boost::future<std::optional<serialization::pimpl::data>> remove_internal(
      const serialization::pimpl::data& key_data) override
    {
        try {
    ...
    boost::future<bool> try_remove_internal(
    ...
    boost::future<bool> try_put_internal(
      const serialization::pimpl::data& key_data,
      ...
      std::chrono::milliseconds timeout) override
    {
        try {
            auto response =
              imap::try_put_internal(key_data, value_data, timeout);
            invalidate_near_cache(key_data);
            return response;
        } catch (exception::iexception&) {
            invalidate_near_cache(key_data);
    ...
    boost::future<std::optional<serialization::pimpl::data>> put_internal(
      const serialization::pimpl::data& key_data,
      const serialization::pimpl::data& value_data,
      ...
      std::chrono::milliseconds ttl) override
    {
        try {
            auto result =
              imap::try_put_transient_internal(key_data, value_data, ttl);
            invalidate_near_cache(key_data);
            return result;
        } catch (exception::iexception&) {
            invalidate_near_cache(key_data);
                           ...
                           std::chrono::milliseconds ttl) override
    {
        try {
            auto previousValue =
              imap::put_if_absent_data(key_data, value_data, ttl);
            invalidate_near_cache(key_data);
            return previousValue;
        } catch (exception::iexception&) {
            invalidate_near_cache(key_data);
      ...
      const serialization::pimpl::data& new_value_data) override
    {
        try {
            auto result =
              proxy::IMapImpl::replace(key_data, value_data, new_value_data);
            invalidate_near_cache(key_data);
            return result;
        } catch (exception::iexception&) {
            invalidate_near_cache(key_data);
    ...
    boost::future<std::optional<serialization::pimpl::data>> replace_internal(
      const serialization::pimpl::data& key_data,
    ...
    boost::future<bool> evict_internal(
      const serialization::pimpl::data& key_data) override
    {
        try {
      ...
      const std::vector<serialization::pimpl::data>& partition_keys) override
    {
        ...
        try {
            ...
            for (auto& key : partition_keys) {
                ...
                if (cached &&
                    internal::nearcache::NearCache<K, V>::NULL_OBJECT !=
                      cached) {
                    result.push_back(
                      std::make_pair(std::move(*key_data), *cached));
                } else {
                    if (invalidate_on_change_) {
                        (*markers)[key_data] =
                          key_state_marker_->try_mark(*key_data);
                    }
            ...
            return imap::get_all_internal(partition_id, remainingKeys)
              .then(boost::launch::sync, [=](boost::future<EntryVector> f) {
                  EntryVector allEntries(result);
                  for (auto& entry : f.get()) {
                      auto key = std::make_shared<serialization::pimpl::data>(
    ...
    boost::future<protocol::ClientMessage> put_all_internal(
      int partition_id,
    ...
    void invalidate_entries(const EntryVector& entries)
    {
        for (auto& entry : entries) {
            invalidate_near_cache(
              std::make_shared<serialization::pimpl::data>(entry.first));
        }
    }
...
private:
    impl::nearcache::KeyStateMarker* get_key_state_marker()
    {
        return std::static_pointer_cast<
                 impl::nearcache::
                   InvalidationAwareWrapper<serialization::pimpl::data, V>>(
                 near_cache_)
      ...
      std::shared_ptr<client::impl::BaseEventHandler>& handler)
    {
        try {
            invalidation_listener_id_ =
              proxy::ProxyImpl::register_listener(
                create_near_cache_entry_listener_codec(), handler)
                .get();
        } catch (exception::iexception& e) {
            HZ_LOG(
              logger_,
              severe,
              boost::str(boost::format("Near Cache is not initialized!!! %1%") %
                         e.what()));
        }
    }
    ...
    public:
          ...
          const std::vector<int64_t>& /* sequences */) override
        {
            for (auto& k : keys) {
                near_cache_->invalidate(
                  std::make_shared<serialization::pimpl::data>(k));
            }
        }
    ...
    class NearCacheEntryListenerMessageCodec
    ...
    create_near_cache_entry_listener_codec()
    {
        entry_event::type listenerFlags = entry_event::type::INVALIDATION;
        return std::shared_ptr<spi::impl::ListenerMessageCodec>(
          new NearCacheEntryListenerMessageCodec(spi::ClientProxy::get_name(),
                                                 listenerFlags));
    }

    void reset_to_unmarked_state(
    ...
    void try_to_put_near_cache(
      const std::shared_ptr<serialization::pimpl::data>& key_data,
    ...
    /*
                    void tryToPutNearCache(const
       std::shared_ptr<serialization::pimpl::data> &key_data, const
       std::shared_ptr<serialization::pimpl::data> &response) {
                        tryToPutNearCacheInternal<serialization::pimpl::data>(key_data,
       response);
                    }
    */

    template<typename VALUETYPE>
    void try_to_put_near_cache_internal(
      const std::shared_ptr<serialization::pimpl::data>& key_data,
    ...
    /**
     * This method modifies the key data internal pointer although it is marked
     * as const
