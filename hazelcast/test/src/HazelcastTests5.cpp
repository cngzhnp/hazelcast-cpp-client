...
protected:
    ...
    int64_t next_call_id(spi::impl::sequence::CallIdSequence& seq,
                         bool is_urgent)
    {
...
TEST_F(CallIdSequenceWithoutBackpressureTest,
       whenNextRepeated_thenKeepSucceeding)
{
...
TEST_F(CallIdSequenceWithBackpressureTest,
       next_whenNoCapacity_thenBlockTillCapacity)
{
...
TEST_F(CallIdSequenceWithBackpressureTest,
       next_whenNoCapacity_thenBlockTillTimeout)
{
...
TEST_F(CallIdSequenceWithBackpressureTest,
       when_overCapacityButPriorityItem_then_noBackpressure)
{
    spi::impl::sequence::CallIdSequenceWithBackpressure sequence(1, 60000);
...
TEST(FailFastCallIdSequenceTest,
     when_overCapacityButPriorityItem_then_noException)
{
    spi::impl::sequence::FailFastCallIdSequence sequence(1);
...
HazelcastServerFactory::read_from_xml_file(const std::string& xml_file_path)
{
    ...
    if (!xmlFile) {
        ...
        BOOST_THROW_EXCEPTION(exception::illegal_state(
          "HazelcastServerFactory::readFromXmlFile", out.str()));
    }
...
TEST_F(ClientExpirationListenerTest, notified_afterExpirationOfEntries)
{
    ...
    expirationListener.on_expired(
      [&expirationEventArrivalCount](entry_event&&) {
          expirationEventArrivalCount.count_down();
      });

    auto registrationId =
      int_map->add_entry_listener(std::move(expirationListener), true).get();
...
TEST_F(ClientListenerFunctionObjectsTest, lambda)
{
    ...
    auto id = map
                ->add_entry_listener(
                  entry_listener().on_added([&called](entry_event&& e) {
...
TEST_F(ClientListenerFunctionObjectsTest, functor)
{
    ...
    auto id = map
                ->add_entry_listener(
                  entry_listener().on_added(std::move(handler)), true)
                .get();
...
TEST_F(ClientListenerFunctionObjectsTest, staticFunction)
{
    ...
    auto id = map
                ->add_entry_listener(
                  entry_listener().on_added(&OnEntryAdded::handler), true)
                .get();
...
TEST_F(ClientListenerFunctionObjectsTest, bind)
{
    ...
    auto handler =
      std::bind(&MyListener::added, &listener, std::placeholders::_1);
...
public:
    ...
    static client_config create_near_cached_map_client_config()
    {
        ...
        config.add_near_cache_config(
          config::near_cache_config(employeesMapName));
        config.add_near_cache_config(config::near_cache_config(imapName));
        config.add_near_cache_config(
          config::near_cache_config(ONE_SECOND_MAP_NAME));
    ...
    static client_config create_near_cached_object_map_client_config()
    {
        ...
        config.add_near_cache_config(
          config::near_cache_config(intMapName, config::OBJECT));
        config.add_near_cache_config(
          config::near_cache_config(employeesMapName, config::OBJECT));
        config.add_near_cache_config(
          config::near_cache_config(imapName, config::OBJECT));
        config.add_near_cache_config(
...
protected:
    void SetUp() override
    {
        ...
        ASSERT_EQ_EVENTUALLY(
          2, context.get_connection_manager().get_active_connections().size());
    }
                                           ...
                                           boost::latch* evict_latch = nullptr)
    {
        return entry_listener()
          ...
          .on_removed(
            [&remove_latch](entry_event&&) { remove_latch.count_down(); })
          .on_updated(
            [&update_latch](entry_event&&) { update_latch.count_down(); })
          .on_expired(
            [&expiry_latch](entry_event&&) { expiry_latch.count_down(); })
    ...
    void validate_expiry_invalidations(std::shared_ptr<imap> map,
                                       std::function<void()> f)
    {
        ...
        auto id =
          map
            ->add_entry_listener(
              make_countdown_listener(dummy, dummy, dummy, expiry), false)
            .get();
        ...
        if (nearCacheStatsImpl) {
            ASSERT_TRUE_EVENTUALLY(
              initialInvalidationRequests <
              nearCacheStatsImpl->get_invalidation_requests());
            // When ttl expires at server, the server sends another
            // invalidation.
            ASSERT_EQ_EVENTUALLY(
              initialInvalidationRequests + 2,
              nearCacheStatsImpl->get_invalidation_requests());
        } else {
            // trigger eviction
            ASSERT_TRUE_EVENTUALLY(
              (!map->get<std::string, std::string>("key1").get().has_value()));
        }
...
TEST_P(ClientMapTest, testIssue537)
{
    ...
    imap_
      ->put<std::string, std::string>("key1", "value1", std::chrono::seconds(2))
      .get();
...
TEST_P(ClientMapTest, testRemoveAll)
{
    ...
    imap_
      ->remove_all(query::like_predicate(
        client_, query::query_constants::THIS_ATTRIBUTE_NAME, "value%"))
      .get();
...
TEST_P(ClientMapTest, testPutAllGetAllWithJson)
{
    ...
    for (int i = 0; i < 100; i++) {
        mapTemp.emplace(std::to_string(i),
                        hazelcast_json_value("{\"value\":\"value_" +
                                             std::to_string(i) + "\"}"));
    }
    ASSERT_EQ(imap_->size().get(), 0);
    imap_->put_all(mapTemp).get();
    ...
    tempSet.insert(std::to_string(1));
    tempSet.insert(std::to_string(3));
    std::unordered_map<std::string, hazelcast_json_value> m2 =
      imap_->get_all<std::string, hazelcast_json_value>(tempSet).get();

    ASSERT_EQ(2U, m2.size());
    ASSERT_EQ(m2.at(std::to_string(1)),
              hazelcast_json_value("{\"value\":\"value_1\"}"));
    ASSERT_EQ(m2.at(std::to_string(3)),
              hazelcast_json_value("{\"value\":\"value_3\"}"));
}

TEST_P(ClientMapTest, testTryPutRemove)
{
    ASSERT_TRUE(
      imap_->try_put("key1", "value1", std::chrono::seconds(1)).get());
    ASSERT_TRUE(
      imap_->try_put("key2", "value2", std::chrono::seconds(1)).get());
    imap_->lock("key1").get();
    imap_->lock("key2").get();

    boost::latch latch1(2);

    std::thread t1([this, &latch1]() {
        bool result =
          imap_->try_put("key1", "value3", std::chrono::seconds(1)).get();
        if (!result) {
            latch1.count_down();
        }
    });
    std::thread t2([this, &latch1]() {
        bool result = imap_->try_remove("key2", std::chrono::seconds(1)).get();
        if (!result) {
            latch1.count_down();
        }
    });
    ...
    ASSERT_EQ("value1",
              (imap_->get<std::string, std::string>("key1").get().value()));
    ASSERT_EQ("value2",
              (imap_->get<std::string, std::string>("key2").get().value()));
...
TEST_P(ClientMapTest, testPutIfAbsent)
{
    ...
    ASSERT_EQ("value1",
              (imap_->put_if_absent<std::string, std::string>("key1", "value3")
                 .get()
                 .value()));
}
...
TEST_P(ClientMapTest, testLock)
{
    ...
    std::thread t([this, &latch1]() {
        imap_->try_put("key1", "value2", std::chrono::milliseconds(1)).get();
        latch1.count_down();
    });
    ASSERT_OPEN_EVENTUALLY(latch1);
    t.join();
...
TEST_P(ClientMapTest, testLockTtl)
{
    ...
    boost::latch latch1(1);
    std::thread t([this, &latch1]() {
        imap_->try_put("key1", "value2", std::chrono::seconds(5)).get();
        latch1.count_down();
    });
    ASSERT_OPEN_EVENTUALLY(latch1);
    t.join();
...
TEST_P(ClientMapTest, testLockTtl2)
{
    ...
    boost::latch latch1(2);
    std::thread t([this, &latch1]() {
        if (!imap_->try_lock("key1").get()) {
            latch1.count_down();
        }
        if (imap_->try_lock("key1", std::chrono::seconds(5)).get()) {
            latch1.count_down();
        }
    });
    ASSERT_OPEN_EVENTUALLY(latch1);
...
TEST_P(ClientMapTest, testTryLock)
{
    ...
    std::thread t1([this, &latch1]() {
        if (!imap_->try_lock("key1", std::chrono::milliseconds(2)).get()) {
            latch1.count_down();
        }
    });
    ...
    std::thread t2([this, &latch2]() {
        if (imap_->try_lock("key1", std::chrono::seconds(20)).get()) {
            latch2.count_down();
        }
    });
...
{
    ASSERT_TRUE(
      imap_->try_lock("key1", std::chrono::seconds(2), std::chrono::seconds(1))
        .get());
    boost::latch latch1(1);
    std::thread t1([this, &latch1]() {
        if (!imap_->try_lock("key1", std::chrono::milliseconds(2)).get()) {
...
TEST_P(ClientMapTest, testTryLockTtlTimeout)
{
    ASSERT_TRUE(
      imap_
        ->try_lock("key1", std::chrono::seconds(2), std::chrono::seconds(200))
        .get());
    boost::latch latch1(1);
    std::thread t1([this, &latch1]() {
        if (!imap_->try_lock("key1", std::chrono::milliseconds(2)).get()) {
...
TEST_P(ClientMapTest, testForceUnlock)
{
    imap_->lock("key1").get();
    boost::latch latch(1);
    std::thread unlocking([this, &latch] {
        imap_->force_unlock("key1").get();
        latch.count_down();
    });
 ...
 * The hazelcast_json_value should be comparable at Java server side to make it
 * work.
 */
TEST_P(ClientMapTest, DISABLED_testJsonValuesWithpaging_predicate)
{
    ...
    auto predicate =
      imap_->new_paging_predicate<std::string, hazelcast_json_value>(
        (size_t)predSize);
    auto values =
      imap_->values<std::string, hazelcast_json_value>(predicate).get();
...
TEST_P(ClientMapTest, testValuesWithPredicate)
{
    ...
    values = int_map_
               ->values<int>(
                 query::instance_of_predicate(client_, "java.lang.Integer"))
               .get();
    ...
    values =
      int_map_
        ->values<int>(query::instance_of_predicate(client_, "java.lang.String"))
        .get();
    ASSERT_EQ(0, (int)values.size());
    ...
    imap_->put<std::string, std::string>("key_111_test", "myvalue_111_test")
      .get();
    imap_->put<std::string, std::string>("key_22_test", "myvalue_22_test")
      .get();
    ...
    std::vector<std::string> strValues =
      imap_
        ->values<std::string>(query::like_predicate(
          client_, query::query_constants::THIS_ATTRIBUTE_NAME, "value1"))
        .get();
    ...
    char sql[100];
    hazelcast::util::hz_snprintf(sql,
                                 50,
                                 "%s BETWEEN 4 and 7",
...
TEST_P(ClientMapTest, testValuesWithpaging_predicate)
{
    ...
    auto predicate2 =
      int_map_->new_paging_predicate<int, int>(5, lessThanTenPredicate);
    ...
    predSize = 2;
    auto predicate3 = int_map_->new_paging_predicate<int, employee>(
      EmployeeEntryComparator(), (size_t)predSize);
    std::vector<employee> result =
      employees_->values<int, employee>(predicate3).get();
...
TEST_P(ClientMapTest, testKeySetWithPredicate)
{
    ...
    // key != 5
    keys = int_map_
             ->key_set<int>(query::not_equal_predicate(
               client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5))
             .get();
    ...
    // key in {4, 10, 19}
    keys = int_map_
             ->key_set<int>(query::in_predicate(
               client_, query::query_constants::KEY_ATTRIBUTE_NAME, 4, 10, 19))
             .get();
    ...
    keys = int_map_
             ->key_set<int>(
               query::instance_of_predicate(client_, "java.lang.String"))
             .get();
    ASSERT_EQ(0, (int)keys.size());
    ...
    std::vector<std::string> strKeys =
      imap_
        ->key_set<std::string>(query::like_predicate(
          client_, query::query_constants::THIS_ATTRIBUTE_NAME, "value1"))
        .get();
...
TEST_P(ClientMapTest, testKeySetWithpaging_predicate)
{
    ...
    predSize = 2;
    auto predicate3 = int_map_->new_paging_predicate<int, employee>(
      EmployeeEntryKeyComparator(), (size_t)predSize);
    std::vector<int> result = employees_->key_set<int>(predicate3).get();
    // since keyset result only returns keys from the server, no ordering based
    // on the value but ordered based on the keys
    ASSERT_EQ(2, (int)result.size());
...
TEST_P(ClientMapTest, testEntrySetWithPredicate)
{
    ...
    std::vector<std::pair<int, int>> entries =
      int_map_->entry_set<int, int>().get();
    ...
    // true_predicate
    entries =
      int_map_->entry_set<int, int>(query::true_predicate(client_)).get();
    ASSERT_EQ(numItems, (int)entries.size());
    std::sort(entries.begin(), entries.end());
    for (int i = 0; i < numItems; ++i) {
        ASSERT_EQ(expected[i], entries[i]);
    ...
    // false_predicate
    entries =
      int_map_->entry_set<int, int>(query::false_predicate(client_)).get();
    ASSERT_EQ(0, (int)entries.size());
    ...
    imap_->put<std::string, std::string>("key_22_test", "myvalue_22_test")
      .get();
    expectedStrEntries[13] =
      std::pair<std::string, std::string>("key_22_test", "myvalue_22_test");

    // like_predicate
    // value LIKE "value1" : {"value1"}
    std::vector<std::pair<std::string, std::string>> strEntries =
      imap_
        ->entry_set<std::string, std::string>(query::like_predicate(
          client_, query::query_constants::THIS_ATTRIBUTE_NAME, "value1"))
        .get();
    ...
    hazelcast::util::hz_snprintf(sql,
                                 ...
                                 query::query_constants::KEY_ATTRIBUTE_NAME);
    entries =
      int_map_->entry_set<int, int>(query::sql_predicate(client_, sql)).get();
...
TEST_P(ClientMapTest, testEntrySetWithpaging_predicate)
{
    ...
    std::vector<std::pair<int, int>> values =
      int_map_->entry_set<int, int>(predicate).get();
    ASSERT_EQ(predSize, (int)values.size());
    for (int i = 0; i < predSize; ++i) {
...
TEST_P(ClientMapTest, testReplace)
{
    ...
    ASSERT_EQ("value1",
              (imap_->replace<std::string, std::string>("key1", "value2")
                 .get()
                 .value()));
    ASSERT_EQ("value2",
              (imap_->get<std::string, std::string>("key1").get().value()));
    ...
    ASSERT_FALSE(
      (imap_->replace<std::string, std::string>("key1", "value1", "value3")
         .get()));
    ASSERT_EQ("value2",
              (imap_->get<std::string, std::string>("key1").get().value()));

    ASSERT_TRUE(
      (imap_->replace<std::string, std::string>("key1", "value2", "value3")
         .get()));
    ASSERT_EQ("value3",
              (imap_->get<std::string, std::string>("key1").get().value()));
}
...
TEST_P(ClientMapTest, testListenerWithPortableKey)
{
    ...
    employee key("a", 1);
    auto id =
      tradeMap->add_entry_listener(std::move(listener), true, key).get();
    employee key2("a", 2);
...
TEST_P(ClientMapTest, testListener)
{
    ...
    auto listener1 =
      make_countdown_listener(latch1Add, latch1Remove, dummy, dummy);
    auto listener2 =
      make_countdown_listener(latch2Add, latch2Remove, dummy, dummy);

    auto listener1ID =
      imap_->add_entry_listener(std::move(listener1), false).get();
    auto listener2ID =
      imap_->add_entry_listener(std::move(listener2), true, "key3").get();
...
TEST_P(ClientMapTest, testListenerWithtrue_predicate)
{
    ...
    auto listener =
      make_countdown_listener(latchAdd, latchRemove, latchUpdate, latchEvict);

    auto listenerId = int_map_
                        ->add_entry_listener(std::move(listener),
                                             query::true_predicate(client_),
                                             false)
                        .get();
    ...
    ASSERT_FALSE(
      (int_map_->get<int, int>(3).get().has_value())); // trigger eviction
...
TEST_P(ClientMapTest, testListenerWithequal_predicate)
{
    ...
    auto listenerId =
      int_map_
        ->add_entry_listener(
          std::move(listener),
          query::equal_predicate(
            client_, query::query_constants::KEY_ATTRIBUTE_NAME, 3),
          true)
        .get();
    ...
    auto listenerId =
      int_map_
        ->add_entry_listener(
          std::move(listener),
          query::between_predicate(
            client_, query::query_constants::KEY_ATTRIBUTE_NAME, 1, 2),
          true)
        .get();
...
TEST_P(ClientMapTest, testListenerWithsql_predicate)
{
    ...
    auto listenerId =
      int_map_
        ->add_entry_listener(
          std::move(listener), query::sql_predicate(client_, "__key < 2"), true)
        .get();
...
TEST_P(ClientMapTest, testListenerWithRegExPredicate)
{
    ...
    auto listenerId =
      imap_
        ->add_entry_listener(
          std::move(listener),
          query::regex_predicate(
            client_, query::query_constants::KEY_ATTRIBUTE_NAME, ".*met.*"),
          true)
        .get();
    ...
    imap_
      ->put<std::string, std::string>("metin", "ozen", std::chrono::seconds(1))
    ...
    ASSERT_FALSE((imap_->get<std::string, std::string>("metin")
                    .get()
                    .has_value())); // trigger eviction
...
TEST_P(ClientMapTest, testListenerWithinstance_of_predicate)
{
    ...
    auto listenerId = int_map_
                        ->add_entry_listener(std::move(listener),
                                             query::instance_of_predicate(
                                               client_, "java.lang.Integer"),
                                             false)
                        .get();
...
TEST_P(ClientMapTest, testListenerWithnot_predicate)
{
    ...
    auto listenerId =
      int_map_->add_entry_listener(std::move(listener), notPredicate, false)
        .get();
...
TEST_P(ClientMapTest, testListenerWithand_predicate)
{
    ...
    // value == 1
    query::equal_predicate equalPred(
      client_, query::query_constants::KEY_ATTRIBUTE_NAME, 1);
    // key < 3 AND key == 1 --> (1, 1)
    query::and_predicate predicate(client_, greaterLessPred, equalPred);
    auto listenerId =
      int_map_->add_entry_listener(std::move(listener), predicate, false).get();
...
TEST_P(ClientMapTest, testListenerWithor_predicate)
{
    ...
    // value == 1
    query::equal_predicate equalPred(
      client_, query::query_constants::THIS_ATTRIBUTE_NAME, 2);
    // key >= 3 OR value == 2 --> (1, 1), (2, 2)
    query::or_predicate predicate(client_, greaterLessPred, equalPred);
    auto listenerId =
      int_map_->add_entry_listener(std::move(listener), predicate, true).get();
...
TEST_P(ClientMapTest, testClearEvent)
{
    ...
    clearListener.on_map_cleared(
      [&latch1](map_event&&) { latch1.count_down(); });

    auto listenerId =
      imap_->add_entry_listener(std::move(clearListener), false).get();
    imap_->put<std::string, std::string>("key1", "value1").get();
    imap_->clear().get();
    ASSERT_OPEN_EVENTUALLY(latch1);
...
TEST_P(ClientMapTest, testEvictAllEvent)
{
    ...
    evictListener.on_map_evicted(
      [&latch1](map_event&& event) { latch1.count_down(); });

    auto listenerId =
      imap_->add_entry_listener(std::move(evictListener), false).get();
    imap_->put<std::string, std::string>("key1", "value1").get();
    imap_->evict_all().get();
    ASSERT_OPEN_EVENTUALLY(latch1);
    imap_->remove_entry_listener(listenerId).get();
...
TEST_P(ClientMapTest, testMapWithPortable)
{
    ...
    entry_view<int, employee> view =
      employees_->get_entry_view<int, employee>(1).get().value();
    ASSERT_EQ(view.value, e);
    ASSERT_EQ(view.key, 1);

    employees_
      ->add_index(config::index_config::index_type::SORTED, std::string("a"))
      .get();
    employees_
...
{
    imap_
      ->put_transient<std::string, std::string>(
        "ali", "veli", std::chrono::milliseconds(1100))
      .get();
    imap_->flush().get();
    ASSERT_EQ(1, imap_->size().get());
    ...
    ASSERT_FALSE(
      (imap_->get<std::string, std::string>("ali").get().has_value()));
}
...
TEST_P(ClientMapTest, testSubmitToKey)
{
    ...
    auto result =
      employees_->submit_to_key<int, int, EntryMultiplier>(4, processor).get();
    ASSERT_TRUE(result.has_value());
    ASSERT_EQ(4 * processor.get_multiplier(), result.value());
}
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithor_predicate)
{
    ...
    /* age == 21 OR age > 25 */
    query::or_predicate orPredicate(
      client_,
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithgreater_less_predicate)
{
    ...
    std::unordered_map<int, std::optional<int>> result =
      employees_
        ->execute_on_entries<int, int, EntryMultiplier>(
          processor,
          query::greater_less_predicate(client_, "a", 25, false, true))
        .get(); // <25 matching
    ...
    result = employees_
               ->execute_on_entries<int, int, EntryMultiplier>(
                 processor,
                 query::greater_less_predicate(client_, "a", 25, true, true))
               .get(); // <=25 matching
    ...
    result = employees_
               ->execute_on_entries<int, int, EntryMultiplier>(
                 processor,
                 query::greater_less_predicate(client_, "a", 25, true, false))
               .get(); // >=25 matching
      ...
      employees_
        ->execute_on_entries<int, int, EntryMultiplier>(
          ...
          query::instance_of_predicate(client_,
                                       "com.hazelcast.client.test.Employee"))
        .get();
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithnot_predicate)
{
    ...
    result = employees_
               ->execute_on_entries<int, int, EntryMultiplier>(
                 processor, notbetween_predicate)
               .get();
...
template<>
struct hz_serializer<test::ClientMapTest::EntryMultiplier>
  : public identified_data_serializer
{
    ...
    static test::ClientMapTest::EntryMultiplier read_data(
...
template<>
struct hz_serializer<test::PartitionAwareInt>
  : public identified_data_serializer
{
public:
    ...
    static void write_data(const test::PartitionAwareInt& object,
...
template<>
struct hz_serializer<test::ClientMapTest::MapGetInterceptor>
  : public identified_data_serializer
{
    static int get_factory_id() { return 666; }
    ...
    static test::ClientMapTest::MapGetInterceptor read_data(
      object_data_input& reader)
    {
        return test::ClientMapTest::MapGetInterceptor(
...
{
    std::size_t operator()(
      const hazelcast::client::test::PartitionAwareInt& object) const noexcept
    {
