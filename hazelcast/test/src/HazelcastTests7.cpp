...
{
protected:
    entry_listener make_add_remove_listener(boost::latch& added_latch,
                                            boost::latch& removed_latch)
    {
        return entry_listener()
          .on_added([&added_latch](entry_event&&) { added_latch.count_down(); })
          .on_removed(
            [&removed_latch](entry_event&&) { removed_latch.count_down(); });
    }
...
{
    ASSERT_TRUE(
      mm->try_lock("key1", std::chrono::seconds(2), std::chrono::seconds(1))
        .get());
    boost::latch latch1(1);
    std::thread([&]() {
        if (!mm->try_lock("key1", std::chrono::milliseconds(500)).get()) {
...
{
    ASSERT_TRUE(
      mm->try_lock("key1", std::chrono::seconds(1), std::chrono::seconds(200))
        .get());
    boost::latch latch1(1);
    std::thread([&]() {
...
protected:
    ...
    static void SetUpTestCase()
    {
        ...
        clientConfig.get_network_config().get_ssl_config().set_cipher_list(
          "HIGH");
#else
...
protected:
    void offer(int number_of_items)
    {
        for (int i = 1; i <= number_of_items; ++i) {
            ASSERT_TRUE(
              q->offer(std::string("item") + std::to_string(i)).get());
        }
    }
...
TEST_F(ClientQueueTest, testListener)
{
    ...
    auto listener = item_listener().on_added(
...
protected:
    ...
    static void SetUpTestCase()
    {
        ...
        client = new hazelcast_client(
          new_client(
            std::move(client_config().set_cluster_name("executor-test")))
            .get());
    }
...
{
    std::shared_ptr<iexecutor_service> service =
      client->get_executor_service(get_test_name()).get();
...
TEST_F(ClientExecutorServiceTest, testSubmitFailingCallableException)
{
    ...
    auto future =
      service->submit<executor::tasks::FailingCallable, std::string>(task)
        .get_future();
...
TEST_F(ClientExecutorServiceTest, testSubmitFailingCallableReasonExceptionCause)
{
    ...
    auto failingFuture =
      service
        ->submit<executor::tasks::FailingCallable, std::string>(
...
TEST_F(ClientExecutorServiceTest, testCallableSerializedOnce)
{
    ...
    std::shared_ptr<iexecutor_service> service =
      client->get_executor_service(name).get();

    executor::tasks::SerializedCounterCallable counterCallable{ 0 };

    auto future =
      service
        ->submit_to_key_owner<executor::tasks::SerializedCounterCallable,
                              int,
                              std::string>(counterCallable, name)
        .get_future();
...
TEST_F(ClientExecutorServiceTest,
       testUnserializableResponse_exceptionPropagatesToClient)
{
    std::string name = get_test_name();
    ...
    executor::tasks::TaskWithUnserializableResponse
...
TEST_F(ClientExecutorServiceTest, testSubmitCallableToMember)
{
    ...
    auto future = service
                    ->submit_to_member<executor::tasks::GetMemberUuidTask,
                                       boost::uuids::uuid>(task, members[0])
                    .get_future();
...
TEST_F(ClientExecutorServiceTest, testSubmitCallableToMembers)
{
    ...
    auto futuresMap =
      service->submit_to_members<executor::tasks::GetMemberUuidTask,
                                 boost::uuids::uuid>(task, members);
...
TEST_F(ClientExecutorServiceTest, testSubmitCallable_withMemberSelector)
{
    ...
    auto f = service
               ->submit<executor::tasks::AppendCallable, std::string>(callable,
                                                                      selectAll)
               .get_future();
...
TEST_F(ClientExecutorServiceTest,
       testSubmitCallableToMembers_withMemberSelector)
{
    std::shared_ptr<iexecutor_service> service =
      client->get_executor_service(get_test_name()).get();
    ...
    auto futuresMap =
      service->submit_to_members<executor::tasks::GetMemberUuidTask,
                                 boost::uuids::uuid>(task, selectAll);
...
TEST_F(ClientExecutorServiceTest, submitCallableToAllMembers)
{
    ...
    auto futuresMap =
      service
        ->submit_to_all_members<executor::tasks::AppendCallable, std::string>(
          callable);
...
TEST_F(ClientExecutorServiceTest, submitCallableToKeyOwner)
{
    ...
    auto f = service
               ->submit_to_key_owner<executor::tasks::AppendCallable,
                                     std::string,
                                     std::string>(callable, "key")
               .get_future();
...
TEST_F(ClientExecutorServiceTest, submitCallablePartitionAware)
{
    ...
    std::shared_ptr<iexecutor_service> service =
      client->get_executor_service(testName).get();
...
TEST_F(ClientExecutorServiceTest, testExecute)
{
    ...
    service->execute(executor::tasks::MapPutPartitionAwareCallable<std::string>(
      testName, "key"));
...
TEST(AwsConfigTest, testClientConfigUsage)
{
    ...
    ASSERT_TRUE(
      clientConfig.get_network_config().get_aws_config().is_enabled());
...
TEST_F(DescribeInstancesTest, testDescribeInstancesTagAndValueSet)
{
    ...
    client::aws::impl::DescribeInstances desc(
      client_config().get_network_config().get_connection_timeout(),
      awsConfig,
      awsConfig.get_host_header(),
      get_logger());
...
{
    std::filebuf fb;
    ASSERT_TRUE(fb.open("hazelcast/test/resources/sample_aws_response.xml",
                        std::ios::in));
...
{
protected:
    void check_address_exist(
      const std::unordered_map<address, address>& addresses,
...
TEST_F(cloud_discovery_test, non_existent_base_url)
{
    ...
    config.get_connection_strategy_config()
      ...
      .set_cluster_connect_timeout(std::chrono::milliseconds(100));
    auto& cloudConfig = config.get_network_config().get_cloud_config();
    config.set_property(client_properties::CLOUD_URL_BASE,
                        "https://my.url.com");
    cloudConfig.enabled = true;
...
TEST_F(cloud_discovery_test, parse_json)
{
    ...
    spi::impl::discovery::cloud_discovery d(
      config,
