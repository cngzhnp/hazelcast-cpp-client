...
protected:
    ...
    void get_and_verify_entries_in_map(
      const std::shared_ptr<replicated_map>& map)
    {
        ...
        execute_for_each([=](size_t i) {
            auto oldEntry = map
                              ->put<std::string, std::string>(
                                std::string("foo-") + std::to_string(i), "bar")
                              .get();
                              ...
                              const std::string value)
    {
        execute_for_each([&](size_t i) {
            map
              ->put<std::string, std::string>(
                std::string("foo-") + std::to_string(i), value)
              .get();
        });
    }

    static bool find_value_for_key(int key,
                                   TEST_VALUES_TYPE& test_values,
                                   int& value)
    {
    ...
    static client_config
    get_client_config_with_near_cache_invalidation_enabled()
    {
        config::near_cache_config nearCacheConfig;
        nearCacheConfig.set_invalidate_on_change(true).set_in_memory_format(
          config::BINARY);
...
protected:
    ...
    static void SetUpTestCase()
    {
        factory = new HazelcastServerFactory(
          ...
          "replicated-map-binary-in-memory-config-hazelcast.xml");
...
{
    std::shared_ptr<replicated_map> map =
      client->get_replicated_map(get_test_name()).get();
...
{
    std::shared_ptr<replicated_map> map1 =
      client->get_replicated_map(get_test_name()).get();
    std::shared_ptr<replicated_map> map2 =
      client2->get_replicated_map(get_test_name()).get();
...
TEST_F(ClientReplicatedMapTest, testRemove)
{
    ...
    execute_for_each([=](size_t index) {
        auto val = map2
                     ->remove<std::string, std::string>(std::string("foo-") +
                                                        std::to_string(index))
                     .get();
        ASSERT_TRUE(val.has_value());
        ASSERT_EQ("bar", val.value());
    ...
    execute_for_each([=](size_t index) {
        ...
        ASSERT_FALSE(
          (map1->get<std::string, std::string>(key).get().has_value()))
          << "Removed value should not exist for map1";
        ASSERT_FALSE(
          (map2->get<std::string, std::string>(key).get().has_value()))
...
TEST_F(ClientReplicatedMapTest, testContainsKey)
{
    ...
    execute_for_each([=](size_t i) {
        ASSERT_TRUE(
          map2->contains_key(std::string("foo-") + std::to_string(i)).get());
    });
...
TEST_F(ClientReplicatedMapTest, testEntrySet)
{
    ...
    for (auto& entry : entrySet2) {
        int value;
        ASSERT_TRUE(find_value_for_key(
          entry.first.get<int>().value(), testValues, value));
...
TEST_F(ClientReplicatedMapTest, testDeregisterListener)
{
    ...
    ASSERT_EQ(boost::cv_status::timeout,
              map_clearedLatch.wait_for(std::chrono::seconds(1)));
}
...
TEST_F(ClientReplicatedMapInvalidation, testNearCacheInvalidation)
{
    ...
    hazelcast_client client1(
      new_client(get_client_config_with_near_cache_invalidation_enabled())
        .get());
...
template<>
struct hz_serializer<test::ClientReplicatedMapTest::SamplePortable>
  : public portable_serializer
{
    ...
    static void write_portable(
      test::ClientReplicatedMapTest::SamplePortable object,
...
protected:
    struct ListenerState
    {
        std::mutex lock;
        std::vector<int> keys;
        std::atomic<int> add_count{ 0 }, remove_count{ 0 }, update_count{ 0 },
          evict_count{ 0 }, map_clear_count{ 0 }, map_evict_count{ 0 };
...
TEST_F(ClientReplicatedMapListenerTest, testEntryAdded)
{
    ...
    replicatedMap->add_entry_listener(make_event_counting_listener(state_))
      .get();
...
{
    auto replicatedMap = client->get_replicated_map(get_test_name()).get();
    replicatedMap->add_entry_listener(make_event_counting_listener(state_), 1)
      .get();
    replicatedMap->put(1, 1).get();
...
protected:
    ...
    void create_near_cache_context()
    {
        ...
        if (near_cached_client_ != nullptr) {
            near_cached_client_->shutdown().get();
        }
        near_cached_client_ =
        ...
        near_cached_map_ =
          near_cached_client_->get_replicated_map(get_test_name()).get();
    ...
    void test_contains_key(bool use_near_cached_map_for_removal)
    {
        ...
        std::shared_ptr<replicated_map>& adapter =
          use_near_cached_map_for_removal ? near_cached_map_
                                          : no_near_cache_map_;
...
TEST_P(BasicClientReplicatedMapNearCacheTest,
       testContainsKey_withUpdateOnNearCacheAdapter)
{
 ...
 */
TEST_P(BasicClientReplicatedMapNearCacheTest,
       testContainsKey_withUpdateOnDataAdapter)
{
    ...
    for (int i = 0; i < DEFAULT_RECORD_COUNT; i++) {
        ...
        if (value.get() != NULL) {
            ...
            ASSERT_EQ((hazelcast::client::internal::nearcache::NearCache<
                        serialization::pimpl::data,
                        serialization::pimpl::data>::NULL_OBJECT),
                      near_cache_->get(key))
 ...
 * Near Cache, even if the Near Cache is full and the eviction is disabled (via
...
TEST_P(BasicClientReplicatedMapNearCacheTest, testNearCacheEviction)
{
    ...
    populate_map();
    no_near_cache_map_
      ->put(DEFAULT_RECORD_COUNT,
            std::string("value-") + std::to_string(DEFAULT_RECORD_COUNT))
      .get();
...
protected:
    ...
    void assert_that_owned_entry_count_equals(replicated_map& client_map,
                                              int64_t expected)
    {
        ASSERT_EQ(expected,
...
const std::string ClientReplicatedMapNearCacheTest::DEFAULT_NEAR_CACHE_NAME =
  "defaultNearCache";
