...
protected:
    ...
    bool is_stats_updated(const std::string& last_statistics_collection_time)
    {
        ...
        if (stats["lastStatisticsCollectionTime"] !=
            last_statistics_collection_time) {
            return true;
        }
        return false;
    }
    ...
    void produce_some_stats(std::shared_ptr<imap>& map)
    {
        ...
        auto invalidationRequests =
          nearCacheStatsImpl->get_invalidation_requests();

        map->put(5, 10).get();

        // 2 invalidation requests: One locally during map->put and one from
        // server
        ASSERT_EQ_EVENTUALLY(invalidationRequests + 2,
                             nearCacheStatsImpl->get_invalidation_requests());
...
TEST_F(ClientStatisticsTest, testClientStatisticsDisabledByDefault)
{
    ...
    client_config clientConfig;
    clientConfig.set_property(client_properties::STATISTICS_PERIOD_SECONDS,
                              "1");
...
TEST_F(ClientStatisticsTest, testClientStatisticsContent)
{
    ...
    ASSERT_TRUE_EVENTUALLY(
      (statsFromServer = get_client_stats_from_server()).success &&
    ...
    ASSERT_NE(std::string::npos,
              statsFromServer.result.find(std::string("clientName=") +
                                          client.get_name()));
    ASSERT_NE(std::string::npos,
              statsFromServer.result.find("lastStatisticsCollectionTime="));
    ASSERT_NE(std::string::npos,
              statsFromServer.result.find("enterprise=false"));
    ASSERT_NE(std::string::npos, statsFromServer.result.find("clientType=CPP"));
    ASSERT_NE(std::string::npos,
              statsFromServer.result.find(std::string("clientVersion=") +
                                          HAZELCAST_VERSION));
    ASSERT_NE(std::string::npos,
              statsFromServer.result.find("clusterConnectionTimestamp="));
    ASSERT_NE(std::string::npos,
              statsFromServer.result.find(std::string("clientAddress=") +
                                          localAddress));
...
TEST_F(ClientStatisticsTest, testStatisticsCollectionNonDefaultPeriod)
{
    ...
    ASSERT_TRUE_EVENTUALLY(previous_stat_time !=
                           get_stats()["clusterConnectionTimestamp"]);
    ...
    // time measured by us after client connection should be greater than the
    // connection time reported by the statistics
    ASSERT_GE(clientConnectionTime, connectionTimeStat)
      << "connectionTimeStat was " << connectionTimeStat
      << ", clientConnectionTime was " << clientConnectionTime << "(" << stats
      << ")";
    ...
    std::string lastStatisticsCollectionTimeString =
      statsMap["lastStatisticsCollectionTime"];
    ASSERT_NO_THROW(
      (boost::lexical_cast<int64_t>(lastStatisticsCollectionTimeString)))
    ...
    statsMap = get_stats();
    lastStatisticsCollectionTimeString =
      statsMap["lastStatisticsCollectionTime"];

    // wait enough time for statistics collection
    ASSERT_TRUE_EVENTUALLY(
      is_stats_updated(lastStatisticsCollectionTimeString));

    statsMap = get_stats();
    ASSERT_EQ(1U, statsMap.count(mapHitsKey))
      << mapHitsKey << " stat should exist (" << to_string(statsMap) << ")";
    ASSERT_EQ("0", statsMap[mapHitsKey])
      << "Expected 0 map hits (" << to_string(statsMap) << ")";
...
TEST_F(ClientStatisticsTest, testStatisticsPeriod)
{
    ...
    // wait enough time for statistics collection
    ASSERT_TRUE_EVENTUALLY(
      is_stats_updated(initialStats["lastStatisticsCollectionTime"]));
...
hz_serializer<test::rb::StartsWithStringFilter>::read_data(
...
TEST_F(RingbufferTest, testAPI)
{
    std::shared_ptr<ringbuffer> rb =
      client->get_ringbuffer(get_test_name() + "2").get();
...
{
    client2_ringbuffer_->add_all(items_,
                                 client::rb::overflow_policy::OVERWRITE);
...
{
    ASSERT_EQ(client2_ringbuffer_->capacity().get(),
              client_ringbuffer_->capacity().get());
}
...
TEST_F(RingbufferTest, add_all)
{
    ...
    items.push_back("bar");
    auto result =
      client_ringbuffer_->add_all(items, client::rb::overflow_policy::OVERWRITE)
        .get();
...
// checks if the max count works. So if more results are available than needed,
// the surplus results should not be read.
TEST_F(RingbufferTest, readMany_maxCount)
{
    ...
    client::rb::read_result_set rs =
      client_ringbuffer_->read_many<std::string>(0, 3, 3).get();
...
TEST_F(RingbufferTest, readManyAsync_withFilter)
{
    ...
    auto rs =
      client_ringbuffer_->read_many<StartsWithStringFilter>(0, 3, 3, &filter)
        .get();
...
namespace test {
#ifdef HZ_BUILD_WITH_SSL
class ssl_test_base : public ClientTest
{
protected:
    static constexpr const char* server1_public_key =
      "hazelcast/test/resources/server1-cert.pem";
    static constexpr const char* server1_ssl_xml =
      "hazelcast/test/resources/hazelcast-ssl-server1.xml";
    static constexpr const char* default_ca_xml =
      "hazelcast/test/resources/hazelcast-default-ca.xml";

    virtual client_config new_ssl_client_config(
      bool set_different_protocol = false) = 0;
    ...
    {
        HazelcastServerFactory f(server1_ssl_xml);
        HazelcastServer instance(f);
        ASSERT_THROW(new_client(new_client_config()).get(),
    ...
    void test_ssl_enabled_is_client_live()
    {
        ...
        config.get_network_config()
          .get_ssl_config()
          .set_enabled(true)
          .set_protocol(config::ssl_protocol::tlsv12)
          .add_verify_file(server1_public_key);
    ...
    {
        HazelcastServerFactory f(default_ca_xml);
        HazelcastServer instance(f);
    ...
    void test_ssl_enabled_dont_trust_self_signed_certificates()
    {
        HazelcastServerFactory f(server1_ssl_xml);
        HazelcastServer instance(f);
        ASSERT_THROW(new_client(new_ssl_client_config()).get(),
                     exception::illegal_state);
    }
    ...
    void test_ssl_enabled_with_protocol_mismatch()
    {
        ...
        ASSERT_THROW(new_client(std::move(config)).get(),
                     exception::illegal_state);
    }
};
...
TEST(ssl_config_test, try_new_api_when_deprecated_api_is_used)
{
    config::ssl_config c;
    c.set_enabled(true);
    ASSERT_THROW(c.set_context(boost::asio::ssl::context(
...
protected:
    ...
    static constexpr const char* server2_public_key =
      "hazelcast/test/resources/server2-cert.pem";
    static constexpr const char* client1_public_key =
      "hazelcast/test/resources/client1-cert.pem";
    static constexpr const char* client1_private_key =
      "hazelcast/test/resources/client1-key.pem";
    static constexpr const char* client2_public_key =
      "hazelcast/test/resources/client2-cert.pem";
    static constexpr const char* client2_private_key =
      "hazelcast/test/resources/client2-key.pem";
};
...
TEST_F(mutual_authentication_test, ma_required_client_and_server_authenticated)
{
    ...
    ctx.use_certificate_file(client1_public_key,
                             boost::asio::ssl::context::pem);
    ctx.use_private_key_file(client1_private_key,
                             boost::asio::ssl::context::pem);
    config.get_network_config().get_ssl_config().set_context(std::move(ctx));
...
TEST_F(mutual_authentication_test, ma_required_server_not_authenticated)
{
    ...
    ASSERT_THROW(new_client(new_ssl_client_config(std::move(ctx))).get(),
                 exception::illegal_state);
}

TEST_F(mutual_authentication_test,
       ma_required_client_and_server_not_authenticated)
{
    auto f = get_server_factory(true);
    HazelcastServer instance(f);
    ...
    boost::asio::ssl::context ctx(
      boost::asio::ssl::context::method::tlsv12_client);
    ctx.load_verify_file(server2_public_key);
    ctx.use_certificate_file(client2_public_key,
                             boost::asio::ssl::context::pem);
    ctx.use_private_key_file(client2_private_key,
                             boost::asio::ssl::context::pem);
...
TEST_F(mutual_authentication_test, ma_optional_client_and_server_authenticated)
{
    ...
    start_and_verify_client(new_ssl_client_config(std::move(ctx)));
}
...
TEST_F(mutual_authentication_test,
       ma_optional_client_and_server_not_authenticated)
{
    auto f = get_server_factory(false);
    HazelcastServer instance(f);
...
protected:
    ...
    get_ciphers(client_config config)
    {
        ...
        hazelcast::client::internal::socket::SSLSocket& socket =
          (hazelcast::client::internal::socket::SSLSocket&)
            aConnection->get_socket();
...
{
    HazelcastServerFactory sslFactory(get_ssl_file_path());
...
TEST_F(ClientConnectionTest, testExcludedCipher)
{
    ...
    std::string unsupportedCipher =
      supportedCiphers[supportedCiphers.size() - 1].name;
    config = get_config(true);
    config.get_network_config().get_ssl_config().set_cipher_list(
      std::string("HIGH:!") + unsupportedCipher);
...
protected:
    ...
    std::unique_ptr<HazelcastServer> start_server(client_config& client_config)
    {
        ...
        if (client_config.get_network_config().get_ssl_config().is_enabled()) {
            return std::unique_ptr<HazelcastServer>(
              new HazelcastServer(ssl_factory_));
        }
#endif
        return std::unique_ptr<HazelcastServer>(
          new HazelcastServer(default_server_factory()));
    }
...
{
    auto c = GetParam()();
    c.get_connection_strategy_config()
      .get_retry_config()
      .set_cluster_connect_timeout(std::chrono::seconds(3));
    ASSERT_THROW(hazelcast::new_client(std::move(c)).get(),
                 exception::illegal_state);
}
...
TEST_P(ClusterTest, testConnectionAttemptPeriod)
{
    ...
    clientConfig.get_network_config().set_connection_timeout(
      std::chrono::milliseconds(500));
    config::connection_retry_config& retry_config =
      clientConfig.get_connection_strategy_config().get_retry_config();
    retry_config.set_cluster_connect_timeout(timeout)
      .set_initial_backoff_duration(std::chrono::milliseconds(100));
...
INSTANTIATE_TEST_SUITE_P(
  All,
  ClusterTest,
  ::testing::Values([]() { return ClientTest::get_config(); },
                    []() { return ClientTest::get_config(true); }));
#else
INSTANTIATE_TEST_SUITE_P(All, ClusterTest, ::testing::Values([]() {
                             return ClientTest::get_config();
                         }));
#endif
} // namespace test
} // namespace client
...
TEST_F(HeartbeatTest, testPing)
{
    ...
    config.set_cluster_name("short-heartbeat-cluster")
      .set_property("hazelcast_client_heartbeat_interval", "1");
...
make_socket_interceptor(boost::latch& l)
{
    return socket_interceptor().on_connect(
      [&l](const hazelcast::client::socket& connected_sock) {
...
TEST_F(ClientAuthenticationTest, testUserPasswordCredentials)
{
    HazelcastServerFactory factory(
      "hazelcast/test/resources/hazelcast-username-password.xml");
...
{
    HazelcastServerFactory factory(
      "hazelcast/test/resources/hazelcast-token-credentials.xml");
    HazelcastServer instance(factory);
...
TEST_F(ClientAuthenticationTest, testIncorrectGroupName)
{
    ...
    ASSERT_THROW((hazelcast::new_client(std::move(config)).get()),
                 exception::illegal_state);
}
} // namespace test
} // namespace client
...
TEST_F(ClientEnpointTest, testConnectedClientEnpoint)
{
    ...
    std::shared_ptr<connection::Connection> connection =
      connectionManager.get_random_connection();
...
{
    HazelcastServer instance(default_server_factory());
    auto hazelcastClient = get_new_client();
...
class PnCounterFunctionalityTest : public ClientTest
{
public:
    static void SetUpTestCase()
    {
...
{
    HazelcastServerFactory factory(
      "hazelcast/test/resources/hazelcast-lite-member.xml");
    HazelcastServer instance(factory);
 ...
 */
class ClientPNCounterConsistencyLostTest : public ClientTest
{
protected:
    ...
    void terminate_member(const member& address,
                          HazelcastServer& server1,
                          HazelcastServer& server2)
    {
...
TEST_F(ClientPNCounterConsistencyLostTest,
       driverCanContinueSessionByCallingReset)
{
    HazelcastServerFactory factory(
      "hazelcast/test/resources/hazelcast-pncounter-consistency-lost-test.xml");
    HazelcastServer instance(factory);
...
protected:
    membership_listener make_membership_listener(boost::latch& joined,
                                                 boost::latch& left)
    {
        return membership_listener()
          .on_joined(
            [&joined](const membership_event&) { joined.count_down(); })
          .on_left([&left](const membership_event&) { left.count_down(); });
    }

    membership_listener make_initial_membership_listener(boost::latch& joined,
                                                         boost::latch& left)
    {
        return membership_listener()
...
{
    HazelcastServer instance(default_server_factory());
    auto hazelcastClient = hazelcast::new_client(GetParam()()).get();
    cluster cluster = hazelcastClient.get_cluster();
    ...
    auto init_listener =
      make_initial_membership_listener(memberAddedInit, memberRemovedInit);
    auto listener = make_membership_listener(memberAdded, memberRemoved);

    auto initialListenerRegistrationId =
      cluster.add_membership_listener(std::move(init_listener));
    auto sampleListenerRegistrationId =
      cluster.add_membership_listener(std::move(listener));

    HazelcastServer instance2(default_server_factory());
...
TEST_P(SimpleListenerTest, testClusterListenersFromConfig)
{
    ...
    HazelcastServer instance(default_server_factory());
    auto hazelcastClient = hazelcast::new_client(std::move(clientConfig)).get();
...
TEST_P(SimpleListenerTest, testDeregisterListener)
{
    ...
    ASSERT_FALSE(map
                   ->remove_entry_listener(
                     spi::ClientContext(hazelcastClient).random_uuid())
                   .get());
...
INSTANTIATE_TEST_SUITE_P(
  ...
  SimpleListenerTest,
  ::testing::Values([]() { return ClientTest::get_config(); },
                    []() { return ClientTest::get_config(false, false); }));
} // namespace test
} // namespace client
} // namespace hazelcast
...
TEST_F(ClientTxnMapTest, testPutGet)
{
    ...
    ASSERT_FALSE(
      (map->put<std::string, std::string>("key1", "value1").get().has_value()));
    ASSERT_EQ("value1",
              (map->get<std::string, std::string>("key1").get().value()));
    auto val =
      client_.get_map(name).get()->get<std::string, std::string>("key1").get();
    ASSERT_FALSE(val.has_value());

    context.commit_transaction().get();

    ASSERT_EQ("value1",
              (client_.get_map(name)
                 .get()
                 ->get<std::string, std::string>("key1")
                 .get()
                 .value()));
}
...
TEST_F(ClientTxnMapTest, testRemove)
{
    ...
    ASSERT_FALSE(
      (map->remove<std::string, std::string>("key2").get().has_value()));
...
TEST_F(ClientTxnMapTest, testReplace)
{
    ...
    ASSERT_EQ("value1",
              (map->replace<std::string, std::string>("key1", "myNewValue")
                 .get()
                 .value()));
...
TEST_F(ClientTxnMapTest, testPutIfSame)
{
    ...
    auto val =
      map->put_if_absent<std::string, std::string>("key1", "value1").get();
    ASSERT_FALSE(val.has_value());
    val = map->get<std::string, std::string>("key1").get();
    ASSERT_TRUE(val.has_value());
    ASSERT_EQ("value1", val.value());
...
//                        pass.set(map->try_put("var", 1, 0, TimeUnit.SECONDS)
//                        == false); latch2.count_down();
//                    } catch (Exception e) {
//                    }
//                }
//            }
...
//            {
//                public Boolean execute(TransactionalTaskContext context)
//                throws transaction {
//                    try {
//                        final TransactionalMap<String, Integer> txMap =
//                        context.getMap("testTxnGetForUpdate");
...
TEST_F(ClientTxnMapTest, testKeySetValues)
{
    ...
    ASSERT_FALSE((txMap->put<std::string, std::string>("key3", "value3")
                    .get()
                    .has_value()));
...
TEST_F(ClientTxnMapTest, testKeySetAndValuesWithPredicates)
{
    ...
    auto txMap = context.get_map(name);
    ASSERT_FALSE(
      (txMap->put<employee, employee>(emp2, emp2).get().has_value()));
...
{
    return membership_listener().on_left(
      [&l](const membership_event&) { l.count_down(); });
}

ClientTxnTest::ClientTxnTest()
  : hazelcast_instance_factory_(default_server_factory())
{
...
{
    client_->shutdown().get();
    ASSERT_THROW(client_->new_transaction_context(),
...
TEST_F(ClientTxnTest, testTxnCommitUniSocket)
{
    client_config clientConfig;
    clientConfig.get_network_config().set_smart_routing(false);
    hazelcast_client uniSocketClient(new_client(std::move(clientConfig)).get());
...
TEST_F(ClientTxnTest, testTxnCommitWithOptions)
{
    ...
    transactionOptions.set_transaction_type(
      transaction_options::transaction_type::TWO_PHASE);
    transactionOptions.set_timeout(std::chrono::seconds(60));
    transactionOptions.set_durability(2);
    transaction_context context =
      client_->new_transaction_context(transactionOptions);
...
TEST_F(ClientTxnMultiMapTest, testRemoveIfExists)
{
    ...
    ASSERT_EQ(3,
              (int)(multiMap->get<std::string, std::string>(key).get().size()));
...
TEST_F(ClientTxnMultiMapTest, testPutGetRemove)
{
    ...
    for (int i = 0; i < n; i++) {
        futures[i] = boost::async([&mm, this, i]() {
            ...
            client_.get_multi_map("testPutGetRemove")
              .get()
              ->put(key, "value")
              .get();
            transaction_context context = client_.new_transaction_context();
            ...
            auto removedValues =
              multiMap->remove<std::string, std::string>(key2).get();
            ASSERT_EQ(2U, removedValues.size());
            ASSERT_TRUE(
              (removedValues[0] == "value21" &&
               removedValues[1] == "value22") ||
              (removedValues[1] == "value21" && removedValues[0] == "value22"));
            context.commit_transaction().get();

            ASSERT_EQ(
              3, (int)(mm->get<std::string, std::string>(key).get().size()));
        });
    }
...
TEST_F(ClientTxnQueueTest, testTransactionalOfferPoll2)
{
    ...
    ASSERT_EQ(
      "item0",
      client_.get_queue("defQueue1").get()->poll<std::string>().get().value());
...
TEST_F(DataInputTest, testReadCharLittleEndian)
{
    std::vector<byte> bytes{ 'a', 'b' };
    serialization::pimpl::data_input<std::vector<byte>> dataInput(
      boost::endian::order::little, bytes);
...
{
    std::vector<byte> bytes{ 0x00, 0x00, 0x00, 0x02, 0x12, 0x34,
                             0x56, 0x78, 0x1A, 0xBC, 0xDE, 0xEF };
    serialization::pimpl::data_input<std::vector<byte>> dataInput(
      boost::endian::order::big, bytes);
...
{
    std::vector<byte> bytes{ 0x00, 0x00, 0x00, 0x02, 0x12, 0x34, 0x56,
                             0x78, 0x9A, 0xBC, 0xDE, 0xEF, 0x11, 0xA2,
...
TEST_F(AddressUtilTest, testParsingHostAndPort)
{
    hazelcast::util::AddressHolder addressHolder =
      hazelcast::util::AddressUtil::get_address_holder(
    ...
    addressHolder = hazelcast::util::AddressUtil::get_address_holder(
      "[::ffff:192.0.2.128]:5700");
    ASSERT_EQ("::ffff:192.0.2.128", addressHolder.get_address());
    ASSERT_EQ(5700, addressHolder.get_port());

    addressHolder =
      hazelcast::util::AddressUtil::get_address_holder("192.168.1.1:5700");
    ASSERT_EQ("192.168.1.1", addressHolder.get_address());
    ASSERT_EQ(5700, addressHolder.get_port());

    addressHolder =
      hazelcast::util::AddressUtil::get_address_holder("hazelcast.com:80");
...
TEST_F(AddressUtilTest, testGetByNameIpV4)
{
    ...
    boost::asio::ip::address address =
      hazelcast::util::AddressUtil::get_by_name(addrString);
...
TEST_F(DataOutputTest, testWriteIntegerArray)
{
    std::vector<byte> bytes{ 0x00, 0x00, 0x00, 0x02, 0x12, 0x34,
                             0x56, 0x78, 0x1A, 0xBC, 0xDE, 0xEF };
    std::vector<int32_t> actualValues{ INT32_C(0x12345678),
                                       INT32_C(0x1ABCDEEF) };
    serialization::pimpl::data_output dataOutput(boost::endian::order::big);
...
{
    std::vector<byte> bytes = { 0x00, 0x00, 0x00, 0x02, 0x12, 0x34, 0x56,
                                0x78, 0x9A, 0xBC, 0xDE, 0xEF, 0x01, 0xA2,
                                0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8 };
    std::vector<int64_t> actualValues{ INT64_C(0x123456789ABCDEEF),
                                       INT64_C(0x01A2A3A4A5A6A7A8) };
    serialization::pimpl::data_output dataOutput(boost::endian::order::big);
...
{
    hazelcast::util::SyncHttpsClient httpsClient(
      "localhost", "non_existentURL/no_page", std::chrono::seconds(5));
...
TEST_F(HttpsClientTest, testConnectToGithub)
{
    hazelcast::util::SyncHttpsClient httpsClient(
      ...
      "/?Action=DescribeInstances&Version=2014-06-15&X-Amz-Algorithm=AWS4-HMAC-"
      "SHA256&X-Amz-Credential=AKIAIU5IAVNR6X75ARYQ%2F20170413%2Fus-east-1%"
      "2Fec2%2Faws4_request&X-Amz-Date=20170413T083821Z&X-Amz-Expires=30&X-Amz-"
      "Signature=dff261333170c81ecb21f3a0d5820147233197a32c&X-Amz-"
      "SignedHeaders=host",
