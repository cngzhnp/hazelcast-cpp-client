    ...
    static void SetUpTestCase()
    {
        if (std::string(
            ...
            factory = new HazelcastServerFactory(
              "hazelcast/test/resources/"
              "hazelcast-cp-sessionless-semaphore.xml");
        } else {
            factory = new HazelcastServerFactory(
              "hazelcast/test/resources/hazelcast-cp.xml");
        }
...
{
protected:
    std::shared_ptr<hazelcast::cp::atomic_long> get_cp_structure(
...
{
    ASSERT_THROW(
      client_->get_cp_subsystem().get_atomic_long("long@METADATA").get(),
...
{
protected:
    std::shared_ptr<hazelcast::cp::atomic_reference> get_cp_structure(
      const std::string& name) override
    {
...
{
    ASSERT_THROW(
      client_->get_cp_subsystem().get_atomic_reference("ref@METADATA").get(),
      exception::illegal_argument);
}
...
TEST_F(basic_atomic_ref_test, test_compare_and_set)
{
    ...
    ASSERT_TRUE(
      cp_structure_->compare_and_set(static_cast<std::string*>(nullptr), &str1)
        .get());
    ASSERT_EQ(str1, *cp_structure_->get<std::string>().get());
    ASSERT_FALSE(
      cp_structure_->compare_and_set(static_cast<std::string*>(nullptr), &str1)
    ...
    ASSERT_TRUE(
      cp_structure_->compare_and_set(&str2, static_cast<std::string*>(nullptr))
        .get());
...
TEST_F(basic_atomic_ref_test, test_contains)
{
    std::string str1("str1");
    ASSERT_TRUE(
      cp_structure_->contains(static_cast<std::string*>(nullptr)).get());
...
TEST_F(basic_atomic_ref_test, test_alter)
{
    ...
    auto val =
      cp_structure_->alter_and_get<std::string>(test::append_string{ "str3" })
        .get();
    ASSERT_TRUE(val);
    ASSERT_EQ("str1str2str3", *val);

    val =
      cp_structure_->get_and_alter<std::string>(test::append_string{ "str4" })
        .get();
    ASSERT_TRUE(val);
    ASSERT_EQ("str1str2str3", *val);
...
TEST_F(basic_atomic_ref_test, test_apply)
{
    ...
    auto val =
      cp_structure_->apply<std::string>(test::append_string{ "str2" }).get();
    ASSERT_TRUE(val);
...
TEST_F(basic_atomic_ref_test, test_create_after_destroy)
{
    ...
    cp_structure_ =
      client_->get_cp_subsystem().get_atomic_reference(name).get();
    ASSERT_THROW(cp_structure_->set(std::string("str1")).get(),
                 exception::distributed_object_destroyed);
}
...
{
protected:
    std::shared_ptr<hazelcast::cp::latch> get_cp_structure(
      const std::string& name) override
    {
...
{
    ASSERT_THROW(cp_structure_->try_set_count(-20).get(),
                 exception::illegal_argument);
}

TEST_F(basic_latch_test, test_try_set_count_when_argument_zero)
{
    ASSERT_THROW(cp_structure_->try_set_count(0).get(),
                 exception::illegal_argument);
}
...
TEST_F(basic_latch_test, test_count_down_after_destroy)
{
    ...
    ASSERT_THROW(cp_structure_->count_down().get(),
                 exception::distributed_object_destroyed);
}
...
{
protected:
    static constexpr size_t LOCK_SERVICE_WAIT_TIMEOUT_TASK_UPPER_BOUND_MILLIS =
      1500; // msecs

    std::shared_ptr<hazelcast::cp::fenced_lock> get_cp_structure(
      const std::string& name) override
    {
    ...
    void close_session(const hazelcast::cp::raft_group_id& group_id,
                       int64_t session_id)
    {
        auto request = client::protocol::codec::cpsession_closesession_encode(
          group_id, session_id);
        auto context = spi::ClientContext(*client_);
        spi::impl::ClientInvocation::create(context, request, "sessionManager")
          ->invoke()
          .get();
    }
    ...
    void test_when_session_closed(std::function<void()> f)
    {
        ...
        auto session_id =
          spi::ClientContext(*client_).get_proxy_session_manager().get_session(
            group_id);
...
{
    ASSERT_THROW(cp_structure_->unlock().get(),
                 exception::illegal_monitor_state);
}

TEST_F(basic_lock_test, test_get_fence_when_free)
{
    ASSERT_THROW(cp_structure_->get_fence().get(),
                 exception::illegal_monitor_state);
}
...
TEST_F(basic_lock_test, test_lock_unlock_then_lock)
{
    ...
    auto f =
      std::async([=]() { return cp_structure_->lock_and_get_fence().get(); });
...
TEST_F(basic_lock_test, test_lock_unlock_when_pending_lock_of_other_thread)
{
    ...
    auto f = std::async([=, &start]() {
        auto invocation =
          cp_structure_->try_lock_and_get_fence(std::chrono::seconds(60));
        start.count_down();
...
{
    auto fence =
      cp_structure_->try_lock_and_get_fence(std::chrono::seconds(1)).get();
...
{
    try_lock(std::chrono::milliseconds(
      LOCK_SERVICE_WAIT_TIMEOUT_TASK_UPPER_BOUND_MILLIS + 1));
}
...
TEST_F(basic_lock_test,
       test_reentrant_try_lock_with_timeout_fails_when_session_closed)
{
    test_when_session_closed(
      [=]() { cp_structure_->try_lock(std::chrono::seconds(1)).get(); });
}
...
TEST_F(basic_lock_test, test_failed_try_lock_does_not_acquire_session)
{
    ...
    auto& session_manager =
      spi::ClientContext(*client_).get_proxy_session_manager();
    auto session_id = session_manager.get_session(group_id);
    ASSERT_NE(
      ::hazelcast::cp::internal::session::proxy_session_manager::NO_SESSION_ID,
      session_id);
    ASSERT_EQ(1,
              session_manager.get_session_acquire_count(group_id, session_id));
...
TEST_F(basic_lock_test, test_destroy)
{
    ...
    ASSERT_THROW(cp_structure_->try_lock().get(),
                 exception::distributed_object_destroyed);
}
...
{
    auto c =
      hazelcast::new_client(std::move(get_config().set_cluster_name(
                              client_->get_client_config().get_cluster_name())))
        .get();
...
{
protected:
    std::shared_ptr<hazelcast::cp::counting_semaphore> get_cp_structure(
      const std::string& name) override
    {
    ...
    {
        return std::move(cp_test::get_client_config().set_cluster_name(
          "sessionless-semaphore"));
    }
};
...
{
    ASSERT_THROW(
      client_->get_cp_subsystem().get_semaphore("semaphore@METADATA").get(),
      exception::illegal_argument);
}
...
TEST_F(basic_sessionless_semaphore_test,
       test_init_fails_when_already_initialized)
{
    ASSERT_TRUE(cp_structure_->init(7).get());
...
TEST_F(basic_sessionless_semaphore_test, test_acquire)
{
    ...
    for (int32_t i = 0; i < number_of_permits; ++i) {
        ASSERT_EQ(number_of_permits - i,
                  cp_structure_->available_permits().get());
...
TEST_F(basic_sessionless_semaphore_test, test_acquire_when_no_permits)
{
    ...
    ASSERT_TRUE_ALL_THE_TIME((f.wait_for(std::chrono::seconds::zero()) ==
...
TEST_F(basic_sessionless_semaphore_test,
       test_acquire_when_no_permits_and_semaphore_destroyed)
{
    ASSERT_TRUE(cp_structure_->init(0).get());
    auto f = std::async([=]() { cp_structure_->acquire().get(); });
...
TEST_F(basic_sessionless_semaphore_test,
       test_allow_negative_permits_juc_compatibility)
{
    ASSERT_TRUE(cp_structure_->init(0).get());
...
TEST_F(basic_sessionless_semaphore_test,
       test_release_when_blocked_acquire_thread)
{
    ASSERT_TRUE(cp_structure_->init(0).get());
...
TEST_F(basic_sessionless_semaphore_test,
       test_multiple_acquire_when_not_enough_permits)
{
...
TEST_F(basic_sessionless_semaphore_test,
       test_multiple_release_when_blocked_acquire_threads)
{
    int32_t number_of_permits = 10;
    ASSERT_TRUE(cp_structure_->init(number_of_permits).get());
...
{
    ASSERT_THROW(cp_structure_->reduce_permits(-5).get(),
                 exception::illegal_argument);
    ASSERT_EQ(0, cp_structure_->available_permits().get());
}
...
{
    ASSERT_THROW(cp_structure_->increase_permits(-5).get(),
                 exception::illegal_argument);
    ASSERT_EQ(0, cp_structure_->available_permits().get());
}
...
TEST_F(basic_sessionless_semaphore_test, test_try_acquire_for)
{
    ...
    for (int32_t i = 0; i < number_of_permits; ++i) {
        ASSERT_EQ(number_of_permits - i,
                  cp_structure_->available_permits().get());
        ASSERT_TRUE(
          cp_structure_->try_acquire_for(std::chrono::seconds(120)).get());
    }
...
TEST_F(basic_sessionless_semaphore_test,
       test_try_acquire_multiple_when_argument_negative)
{
    ASSERT_TRUE(cp_structure_->init(0).get());
    ASSERT_THROW(cp_structure_->try_acquire(-5).get(),
                 exception::illegal_argument);
    ASSERT_EQ(0, cp_structure_->available_permits().get());
}
...
{
    ASSERT_NO_THROW(cp_structure_->destroy().get());
    ASSERT_THROW(cp_structure_->init(1).get(),
                 exception::distributed_object_destroyed);
}
...
TEST_F(basic_sessionless_semaphore_test, test_acquire_on_multiple_proxies)
{
    hazelcast_client client2(
      new_client(std::move(client_config().set_cluster_name(
                   client_->get_client_config().get_cluster_name())))
        .get());
    auto semaphore2 =
      client2.get_cp_subsystem().get_semaphore(cp_structure_->get_name()).get();
...
TEST_F(basic_session_semaphore_test,
       test_acquire_when_no_permits_and_semaphore_destroyed)
{
    ASSERT_TRUE(cp_structure_->init(0).get());
    auto f = std::async([=]() { cp_structure_->acquire().get(); });
...
TEST_F(basic_session_semaphore_test, test_acquire_after_release)
{
    ...
    auto f =
      std::async([=]() { ASSERT_NO_THROW(cp_structure_->release().get()); });
...
TEST_F(basic_session_semaphore_test, test_multiple_acquires_after_release)
{
    ...
    // This sleep exists at Java test but sleep is not good and reliable in
    // tests and vital here.
